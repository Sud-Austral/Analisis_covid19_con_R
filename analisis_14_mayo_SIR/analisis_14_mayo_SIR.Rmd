...
covid19.metro = covid19[covid19$Comuna=="Cerrillos",]
...
```
#### Estimación del Valor $R_0$

```{r}

x=tabla.santiago$tabla.metro.Recuperados_Diarios
y=pop_santiago*log(tabla.santiago$susceptibles)
summary(lm(y~x))

(estimacion.R0 = -summary(lm(y~x))$coefficients[2])

dia.ultimo = length(tabla.santiago[,1])
exp(estimacion.R0*tabla.santiago$tabla.metro.Recuperados_Diarios[dia.ultimo]/pop_santiago)

```
#### Conclusión

El Covid19 se expandirá en forma terrible sobre la comuna de Santiago, pero el valor $R^2$ es muy pequeño, por lo que la estimación no resulta muy precisa.
